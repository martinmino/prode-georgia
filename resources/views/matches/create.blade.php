<x-app-layout>
    ...
    <form class="row g-3" action="{{ route('matches.store') }}" method="POST">
        ...
        <div class="col-md-3">
            ...
            <input type="time" class="form-control {{ $errors->has('time') ? 'is-invalid' : '' }}" id="time"
        ...
        <div class="col-md-3">
            ...
            <input type="text" class="form-control {{ $errors->has('group') ? 'is-invalid' : '' }}" id="group"
        ...
        <div class="col-md-3">
            ...
            <input type="text" class="form-control {{ $errors->has('phase') ? 'is-invalid' : '' }}" id="phase"
                name="phase" value="{{ old('phase') }}">
            @error('phase')
                <div class="invalid-feedback">
                    {{ $message }}
                </div>
        ...
        <div class="col-md-6">
            ...
            <select class="form-select from-control {{ $errors->has('country1_id') ? 'is-invalid' : '' }}"
                id="country1_id" name="country1_id">
                <option value="">---Seleccione un país---</option>
                @foreach ($countries as $item)
                    <option value="{{ $item->id }}" {{ old('country1_id') == $item->id ? 'selected' : '' }}>
                        {{ $item->name }}</option>
                @endforeach
            </select>
            @error('country1_id')
        ...
        <div class="col-md-6">
            ...
            <select class="form-select from-control {{ $errors->has('country2_id') ? 'is-invalid' : '' }}"
                id="country2_id" name="country2_id">
                <option value="">---Seleccione un país---</option>
                @foreach ($countries as $item)
                    <option value="{{ $item->id }}" {{ old('country2_id') == $item->id ? 'selected' : '' }}>
                        {{ $item->name }}</option>
                @endforeach
            </select>
            @error('country2_id')
        ...
        <div class="col-3">
            ...
            <input type="date" class="form-control {{ $errors->has('active_since') ? 'is-invalid' : '' }}"
                id="active_since" name="active_since" value="{{ old('active_since') }}">
            @error('active_since')
                <div class="invalid-feedback">
                    {{ $message }}
                </div>
        ...
        <div class="col-12">
            <div class="form-check">
                <input class="form-check-input" type="checkbox" name="penalties_definition" value="penalties_definition"
