<x-app-layout>
    ...
    <form class="row g-3" action="{{ route('matches.update', $match->id) }}" method="POST">
        ...
        <div class="col-md-3">
            ...
            <input type="text" class="form-control {{ $errors->has('group') ? 'is-invalid' : '' }}" id="group" name="group"
                value="{{ $match->group }}">
            @error('group')
                <div class="invalid-feedback">
                    {{ $message }}
        ...
        <div class="col-md-3">
            ...
            <input type="text" class="form-control {{ $errors->has('phase') ? 'is-invalid' : '' }}" id="phase" name="phase"
                value="{{ $match->phase }}">
            @error('phase')
                <div class="invalid-feedback">
                    {{ $message }}
        ...
        <div class="col-md-4">
            ...
            <select class="form-select from-control {{ $errors->has('country1_id') ? 'is-invalid' : '' }}" id="country1_id"
                ...
                @foreach ($countries as $item)
                    <option value="{{ $item->id }}" {{ $match->country1_id == $item->id ? 'selected' : '' }}>
                        {{ $item->name }}</option>
                @endforeach
            </select>
            @error('country1_id')
        ...
        <div class="col-md-1">
            ...
            <input type="number" class="form-control {{ $errors->has('goals1') ? 'is-invalid' : '' }}" id="goals1"
                name="goals1" value="{{ $match->goals1 }}" min="0" max="99">
            @error('goals1')
                <div class="invalid-feedback">
                    {{ $message }}
                </div>
        ...
        <div class="col-md-4">
            ...
            <select class="form-select from-control {{ $errors->has('country2_id') ? 'is-invalid' : '' }}" id="country2_id"
                ...
                <option value="">---Seleccione un país---</option>
                @foreach ($countries as $item)
                    <option value="{{ $item->id }}" {{ $match->country2_id == $item->id ? 'selected' : '' }}>
                        {{ $item->name }}</option>
                @endforeach
            </select>
            @error('country2_id')
        ...
        <div class="col-md-1">
            <label for="group" class="form-label">Goles</label>
            <input type="number" class="form-control {{ $errors->has('goals2') ? 'is-invalid' : '' }}" id="goals2"
                name="goals2" value="{{ $match->goals2 }}" min="0" max="99">
            @error('goals2')
                <div class="invalid-feedback">
                    {{ $message }}
                </div>
        ...
        <div class="col-3">
            ...
            <input type="date" class="form-control {{ $errors->has('active_since') ? 'is-invalid' : '' }}"
                id="active_since" name="active_since" value="{{ $match->active_since }}">
            @error('active_since')
                <div class="invalid-feedback">
                    {{ $message }}
                </div>
